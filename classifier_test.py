...
def train(dataset, filename, n_hidden=50, batch_size=10, epochs=2, learning_rate=0.01, model='nn', l2_ratio=1e-7,
          rtn_layer=True):
    ...
    print ("Building model with {} training data, {} classes...".format(len(train_x), n_out),file=filename)
    input_var = T.matrix('x')
    target_var = T.ivector('y')
    if model == 'nn':
        print ("Using neural network...",file=filename)
        net = get_nn_model(n_in, n_hidden, n_out)
    else:
        print ("Using softmax regression...",file=filename)
    ...
    print ("Training Accuracy: {}".format(accuracy_score(train_y, pred_y)),file=filename)
    print (classification_report(train_y, pred_y),file=filename)
    ...
    if test_x is not None:
        ...
        #print("testing size: {}".format(len(test_x)))
        for input_batch, _ in iterate_minibatches(test_x, test_y, batch_size, shuffle=False):
            pred = test_fn(input_batch)
            pred_y.append(np.argmax(pred, axis=1))
        pred_y = np.concatenate(pred_y)
        print ("Testing Accuracy: {}".format(accuracy_score(test_y, pred_y)),file=filename)
        print (classification_report(test_y, pred_y),file=filename)
